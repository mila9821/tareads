...
public class DestinosAdaptadores extends BaseAdapter implements Filterable {

    // declarando variables para usarlo
    Context context; //almacenar card view
    ArrayList<DestinoItems.destinosItems> mdatos; //(llamar datos) el array que se creo en la otra calse , se usa con nombre mdatos
    LayoutInflater inflater; //es un fragmente remplzable dentro del activity de manera dinamica
    DestinosAdaptadores.CustomerFilter filtro; //filtra
    ArrayList<DestinoItems.destinosItems> filtroList; // (filtar datos)

    //constructor para llamar datos
    public DestinosAdaptadores(Context context,ArrayList<DestinoItems.destinosItems> datos){
    ...
    public Filter getFilter() {
        if(filtro ==  null){
            filtro = new DestinosAdaptadores.CustomerFilter(); //filtro del cliente
        }
        return filtro;
    }
    private class CustomerFilter extends  Filter{
        ...
        protected Filter.FilterResults performFiltering(CharSequence constraint) {
            ...
            if(constraint != null && constraint.length()>0){
                ...
                ArrayList<DestinoItems.destinosItems> filtro = new ArrayList<DestinoItems.destinosItems>();

                for(Integer i = 0; i<filtroList.size(); i++){
                    if(filtroList.get(i).getTitulo().toUpperCase().contains(constraint)){
                        DestinoItems.destinosItems
                                c= new DestinoItems.destinosItems(
        ...
        protected void publishResults(CharSequence constraint, Filter.FilterResults results) {
            mdatos = (ArrayList<DestinoItems.destinosItems>) results.values;
