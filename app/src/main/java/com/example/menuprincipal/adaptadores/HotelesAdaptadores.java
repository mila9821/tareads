...
public class HotelesAdaptadores extends BaseAdapter implements Filterable {

    // declarando variables para usarlo
    Context context; //almacenar card view
    ArrayList<HotelesItems.hotelesItems> mdatos; //(llamar datos) el array que se creo en la otra calse , se usa con nombre mdatos
    LayoutInflater inflater; //es un fragmente remplzable dentro del activity de manera dinamica
    HotelesAdaptadores.CustomerFilter filtro; //filtra
    ArrayList<HotelesItems.hotelesItems> filtroList; // (filtar datos)

    //constructor para llamar datos
    public HotelesAdaptadores(Context context,ArrayList<HotelesItems.hotelesItems> datos){
    ...
    public Filter getFilter() {
        if(filtro ==  null){
            filtro = new HotelesAdaptadores.CustomerFilter(); //filtro del cliente
        }
        return filtro;
    }
    private class CustomerFilter extends  Filter{
        ...
        protected Filter.FilterResults performFiltering(CharSequence constraint) {
            ...
            if(constraint != null && constraint.length()>0){
                ...
                ArrayList<HotelesItems.hotelesItems> filtro = new ArrayList<HotelesItems.hotelesItems>();

                for(Integer i = 0; i<filtroList.size(); i++){
                    if(filtroList.get(i).getTitulo().toUpperCase().contains(constraint)){
                        HotelesItems.hotelesItems
                                c= new HotelesItems.hotelesItems(
        ...
        protected void publishResults(CharSequence constraint, Filter.FilterResults results) {
            mdatos = (ArrayList<HotelesItems.hotelesItems>) results.values;
