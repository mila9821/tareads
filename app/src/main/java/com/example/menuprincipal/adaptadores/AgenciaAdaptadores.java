...
public class AgenciaAdaptadores extends BaseAdapter implements Filterable {

    // declarando variables para usarlo
    Context context; //almacenar card view
    ArrayList<AgenciaItems.agenciasItems> mdatos; //(llamar datos) el array que se creo en la otra calse , se usa con nombre mdatos
    LayoutInflater inflater; //es un fragmente remplzable dentro del activity de manera dinamica
    AgenciaAdaptadores.CustomerFilter filtro; //filtra
    ArrayList<AgenciaItems.agenciasItems> filtroList; // (filtar datos)

    //constructor para llamar datos
    public AgenciaAdaptadores(Context context,ArrayList<AgenciaItems.agenciasItems> datos){
    ...
    private class CustomerFilter extends  Filter{
        ...
        protected Filter.FilterResults performFiltering(CharSequence constraint) {
            ...
            if(constraint != null && constraint.length()>0){
                ...
                ArrayList<AgenciaItems.agenciasItems> filtro = new ArrayList<AgenciaItems.agenciasItems>();

                for(Integer i = 0; i<filtroList.size(); i++){
                    if(filtroList.get(i).getTitulo().toUpperCase().contains(constraint)){
                        AgenciaItems.agenciasItems
                                c= new AgenciaItems.agenciasItems(
        ...
        protected void publishResults(CharSequence constraint, Filter.FilterResults results) {
            mdatos = (ArrayList<AgenciaItems.agenciasItems>) results.values;
