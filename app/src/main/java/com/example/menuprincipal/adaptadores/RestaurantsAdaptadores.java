...
public class RestaurantsAdaptadores extends BaseAdapter implements Filterable {

    // declarando variables para usarlo
    Context context; //almacenar card view
    ArrayList<RestaurantsItems.restaurantesItems> mdatos; //(llamar datos) el array que se creo en la otra calse , se usa con nombre mdatos
    LayoutInflater inflater; //es un fragmente remplzable dentro del activity de manera dinamica
    RestaurantsAdaptadores.CustomerFilter filtro; //filtra
    ArrayList<RestaurantsItems.restaurantesItems> filtroList; // (filtar datos)

    //constructor para llamar datos
    public RestaurantsAdaptadores(Context context, ArrayList<RestaurantsItems.restaurantesItems> datos) {
    ...
    public Filter getFilter() {
        if (filtro == null) {
            filtro = new RestaurantsAdaptadores.CustomerFilter(); //filtro del cliente
        }
        return filtro;
    }
    ...
    private class CustomerFilter extends Filter {
        ...
        protected Filter.FilterResults performFiltering(CharSequence constraint) {
            ...
            if (constraint != null && constraint.length() > 0) {
                ...
                ArrayList<RestaurantsItems.restaurantesItems> filtro = new ArrayList<RestaurantsItems.restaurantesItems>();

                for (Integer i = 0; i < filtroList.size(); i++) {
                    if (filtroList.get(i).getTitulo().toUpperCase().contains(constraint)) {
                        RestaurantsItems.restaurantesItems
                                c = new RestaurantsItems.restaurantesItems(
        ...
        protected void publishResults(CharSequence constraint, Filter.FilterResults results) {
            mdatos = (ArrayList<RestaurantsItems.restaurantesItems>) results.values;
